...
function Login({ onBackHome, onGoAdmin }) {
  ...
  const isWorthyAdminEmail = (adminEmail) => {
  ...
  return (
    <main className="loginPage">
      <section className="loginCard">
        <button className="loginBackButton" onClick={onBackHome}>
          ← Back Home
        </button>

        <div className="loginHeader">
          <p className="loginEyebrow">
            {mode === 'signup' ? 'Join Worthy' : mode === 'adminSignup' ? 'Create admin access' : mode === 'admin' ? 'Admin access' : 'Welcome back to Worthy'}
          </p>
          <h2>
            {mode === 'signup' ? 'Create your account' : mode === 'adminSignup' ? 'Create admin account' : mode === 'admin' ? 'Admin login' : 'Login to your account'}
          </h2>
          <p>
            {mode === 'signup'
        ...
        <form className="loginForm">
          ...
          <label>
            ...
            <input
              type="email"
              placeholder={mode === 'admin' || mode === 'adminSignup' ? 'example@worthy.com' : 'Enter your email'}
              value={email}
          ...
          <label>
            ...
            <input
              type="password"
              placeholder={mode === 'admin' || mode === 'adminSignup' ? 'Enter admin password' : 'Enter your password'}
              value={password}
              onChange={(event) => setPassword(event.target.value)}
              autoComplete={mode === 'signup' || mode === 'adminSignup' ? 'new-password' : 'current-password'}
            />
          </label>
          ...
          <button
            ...
            onClick={mode === 'signup' ? handleCreateAccount : mode === 'adminSignup' ? handleCreateAdminAccount : mode === 'admin' ? handleAdminLogin : handleUserLogin}
          >
            {mode === 'signup' ? 'Create account' : mode === 'adminSignup' ? 'Create admin account' : mode === 'admin' ? 'Login as admin' : 'Login'}
          </button>
        </form>
        ...
        {(mode === 'admin' || mode === 'adminSignup') && (
          <div className="signupBox">
            <p>{mode === 'adminSignup' ? 'Already have an admin account?' : 'Need admin access?'}</p>
            <button type="button" onClick={() => switchMode(mode === 'adminSignup' ? 'admin' : 'adminSignup')}>
        ...
        <div className="adminAccessBox">
          <p>{mode === 'admin' || mode === 'adminSignup' ? 'Want to return to customer login?' : 'For business management'}</p>
          <button
            type="button"
            className="adminLoginButton"
            onClick={() => switchMode(mode === 'admin' || mode === 'adminSignup' ? 'login' : 'admin')}
          >
